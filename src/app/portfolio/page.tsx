...
type CardLanguage = 'EN' | 'DE' | 'JP' | 'KR';

const LANG_FLAG: Record<CardLanguage, string> = {
  EN: '🇬🇧', DE: '🇩🇪', JP: '🇯🇵', KR: '🇰🇷',
};
...
const RANGE_DAYS: Record<'1D' | '1W' | '1M' | '3M' | '1Y', number> = {
...
function normalizeHolding(h: Partial<PortfolioHolding> & { cardId: string }): PortfolioHolding {
  return {
    language: 'EN',
    purchaseDate: '',
    setCode: '',
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored) as Array<Partial<PortfolioHolding> & { cardId: string }>;
...
function AddCardModal({
  ...
  const total =
    purchasePrice && qty > 0 ? parseFloat(purchasePrice) * qty : null;
            ...
            <div className="-mx-1">
              ...
              <div className="space-y-0.5 max-h-72 overflow-y-auto">
                {suggestions.map((s) => (
                  ...
                  >
                    <img src={s.imageUrl} alt={s.name} className="h-12 w-9 object-contain rounded shrink-0" />
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right shrink-0">
                      {s.price > 0
                        ? <p className="text-sm font-bold text-gray-900">{formatEur(s.price)}</p>
          ...
          {selected && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-4 bg-violet-50 rounded-2xl">
                ...
                <button onClick={() => setSelected(null)} className="text-gray-300 hover:text-gray-500 transition-colors shrink-0 self-start">
                  <X size={16} />
                </button>
              </div>
