...
import { Upload, Scissors, Play, Share2, Loader2, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react';
  ...
  async function handleProcess() {
    ...
    try {
      const res = await fetch('/api/video/process', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: videoPath, clipDuration, description }),
      });
    ...
    <div className="space-y-4">
      ...
      {step === 'pick' && (
        ...
        >
          ...
          />
          <div className="mt-4">
            <label className="text-xs text-gray-500 block mb-1 text-left">Beschreibung (optional, verbessert die Caption)</label>
            <input
              type="text"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              onClick={(e) => e.stopPropagation()}
              placeholder="z.B. Pokémon 151 Boosterbox – 36 Packs"
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Wähle wie viele Sekunden vom Ende des Videos</p>
            </div>
          </div>

          <div>
            <div className="flex justify-between text-xs text-gray-500 mb-2">
              <span>Letzten Sekunden</span>
