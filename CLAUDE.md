...
26. **Zahlen NIE mit `toFixed()` anzeigen — die Seite ist deutsch** → Preise wurden an ~15 Stellen mit `toFixed(2)` gesetzt und dadurch englisch ausgegeben: „235.71 €" statt „235,71 €", bei teuren Karten „4184.60 €" ohne Tausenderpunkt. Eine korrekte `Intl`-Umsetzung existierte in `portfolio.ts`, war aber nur dort im Einsatz — klassische Doppel-Implementierung. **Regel:** Für JEDE sichtbare Zahl `formatEur` / `formatEurRounded` / `formatPercent` / `formatAmount` (Beträge) / `formatCount` (Stückzahlen) aus `src/lib/format.ts` verwenden. **`formatAmount` ≠ `formatCount`**: Ersterer setzt zwei Nachkommastellen — auf der Startseite stand damit „14.985,00 Karten · 155,00 Sets“. `toFixed()` ist nur erlaubt für: JSON-LD (schema.org verlangt Punkt), Werte in Eingabefeldern (müssen parsebar bleiben) und SVG-Koordinaten. Achtung: `Intl` setzt ein geschütztes Leerzeichen (U+00A0) vor das €-Zeichen — beim Testen explizit berücksichtigen.
